...
export const handler: Handler<TranscriptionEvent, TranscriptionResponse> = async (event) => {
...
const getLanguageCode = (langCode: string): LanguageCode => {
...
async function startTranscriptionJob(event: TranscriptionEvent): Promise<TranscriptionResponse> {
  const { 
    audioFileKey, 
    jobName, 
    languageCode = 'en-US',
...
async function getTranscriptionJobStatus(event: TranscriptionEvent): Promise<TranscriptionResponse> {
  const { jobName } = event;
...
async function processTranscriptionResult(event: TranscriptionEvent): Promise<TranscriptionResponse> {
  ...
  try {
    ...
    if (statusResult.status === 'COMPLETED' && statusResult.transcriptionText) {
      ...
      const processedTranscription = await processTranscriptionText(
        statusResult.transcriptionText,
        statusResult.speakerLabels || []
      );
...
async function fetchTranscriptionText(transcriptFileUri: string): Promise<{
  transcript: string;
  speakerLabels: any[];
} | null> {
  try {
...
async function processTranscriptionText(text: string, speakerLabels: any[]) {
...
async function saveProcessedTranscription(jobName: string, processedData: any, userId?: string) {
  const bucketName = process.env.STORAGE_BUCKET_NAME!;
...
async function logTranscriptionEvent(eventType: string, eventData: any) {
  const logEntry = {
